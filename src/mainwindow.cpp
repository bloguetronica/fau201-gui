    ...
    if (!filename.isEmpty())  // Note that the previous dialog will return an empty string if the user cancels it
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    QMessageBox::critical(this, tr("Error"), tr("The selected file is not a FAU200 settings file."));
                }
                else
                {
...
void MainWindow::on_actionSerial_triggered()
{
    ...
    if (serial.exec() == QDialog::Accepted) {  // Refactored in version 2.1
        QString title = tr("FAU201 Power Supply");
